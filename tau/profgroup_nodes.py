...
def soup2dic(soup, pg_dic):
    ...
    - `exec_time`:
    - `mpiplist`: MPI Processor Name list
    """
    start_time = 0
    end_time = 0
    mpiplist = []
    for attr in soup.findAll("attribute"):
        ...
        if attrname == "MPI Processor Name":
            mpiplist.append(attrvalue)
    pg_dic["exec_time"] = (end_time - start_time) / 1e6
    return pg_dic, pg_dic["exec_time"], mpiplist
...
def main(argv):
    ...
    nodeset = set(nodes)
    li = []
    [li.append(soup2dic(l.soup, dict()))  for l in profs]
    for s in li:
        print s[2][0]
