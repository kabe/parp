...
for x in $PROFS;do
    $CMD --place ha8000 --library "MPICH2 MX GCC HSFS" $HA8000_SOLVER_BASE/$x.batch1 $HA8000_SOLVER_BASE/$MAP
done

MAP=solver_mpich2-mx-gcc-nfs.map
PROFS=`seq 670974 670983`
for x in $PROFS;do
    $CMD --place ha8000 --library "MPICH2 MX GCC NFS" $HA8000_SOLVER_BASE/$x.batch1 $HA8000_SOLVER_BASE/$MAP
done

MAP=solver_mpich2-mx-gcc-lustre.map
PROFS=`seq 671109 671118`
for x in $PROFS;do
    $CMD --place ha8000 --library "MPICH2 MX GCC LUSTRE" $HA8000_SOLVER_BASE/$x.batch1 $HA8000_SOLVER_BASE/$MAP
done

MAP=solver_openmpi-mx-gcc-hsfs.map
PROFS="`seq 671190 671197` `seq 671199 671200`"
for x in $PROFS;do
    $CMD --place ha8000 --library "OPENMPI MX GCC HSFS" $HA8000_SOLVER_BASE/$x.batch1 $HA8000_SOLVER_BASE/$MAP
done

MAP=solver_openmpi-mx-gcc-nfs.map
PROFS="`seq 671304 671310` `seq 671312 671314`"
for x in $PROFS;do
    $CMD --place ha8000 --library "OPENMPI MX GCC NFS" $HA8000_SOLVER_BASE/$x.batch1 $HA8000_SOLVER_BASE/$MAP
done

MAP=solver_openmpi-mx-gcc-lustre.map
PROFS=`seq 671435 671444`
for x in $PROFS;do
    $CMD --place ha8000 --library "OPENMPI MX GCC LUSTRE" $HA8000_SOLVER_BASE/$x.batch1 $HA8000_SOLVER_BASE/$MAP
done
